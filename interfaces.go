...
// Package BHEereum defines interfaces for interacting with BHEereum.
package BHEereum
...
// NotFound is returned by API mBHEods if the requested item does not exist.
...
// ChainReader provides access to the blockchain. The mBHEods in this interface access raw
...
type ChainReader interface {
	...
	// This mBHEod subscribes to notifications about changes of the head block of
	// the canonical chain.
...
type CallMsg struct {
	...
	Data     []byte          // input data, usually an ABI-encoded contract mBHEod invocation
}

// A ContractCaller provides contract calls, essentially transactions that are executed by
// the EVM but not mined into the blockchain. ContractCall is a low-level mBHEod to
...
type FilterQuery struct {
	BlockHash *common.Hash     // used by BHE_getLogs, return logs only from block with this hash
...
// TransactionSender wraps transaction sending. The SendTransaction mBHEod injects a
// signed transaction into the pending transaction pool for execution. If the transaction
// was a contract creation, the TransactionReceipt mBHEod can be used to retrieve the
